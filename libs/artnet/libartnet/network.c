...
static int get_ifaces(iface_t **if_head) {
  ...
  *if_head = if_tail = NULL;
  ...
  for (ptr = buf; ptr < buf + ifc.ifc_len;) {
    ...
    len = max_c(sizeof(struct sockaddr), ifr->ifr_addr.sa_len);
#else
...
int artnet_net_init(node n, const char *preferred_ip) {
  ...
  {
    n->state.ip_addr.s_addr = inet_addr(preferred_ip);
    n->state.bcast_addr.s_addr = inet_addr("255.255.255.255");
    memcpy(&n->state.hw_addr, "FF:FF:FF:FF:FF:FF", ARTNET_MAC_SIZE);
    return ARTNET_EOK;
  }
