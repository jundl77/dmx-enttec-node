...
int TestPlay() {
	...
	std::string out_device_id = "{0.0.0.00000000}.{75e827d5-9c89-4bdd-afbb-1d8b33015814}";
	bool in_raw = false;
	bool out_raw = false;
	backend = SoundIoBackendWasapi;

	double microphone_latency = 0.05; // seconds
	...
	if (!in_device_id.empty()) {
		bool found = false;
		for (int i = 0; i < soundio_input_device_count(soundio); i += 1) {
			struct SoundIoDevice *device = soundio_get_input_device(soundio, i);
			if (device->is_raw == in_raw && strcmp(device->id, in_device_id.c_str()) == 0) {
				in_device_index = i;
				found = true;
		...
		if (!found)
			panic("invalid input device id: %s", in_device_id.c_str());
	}

	int out_device_index = default_out_device_index;
	if (!out_device_id.empty()) {
		bool found = false;
		for (int i = 0; i < soundio_output_device_count(soundio); i += 1) {
			struct SoundIoDevice *device = soundio_get_output_device(soundio, i);
			if (device->is_raw == out_raw && strcmp(device->id, out_device_id.c_str()) == 0) {
				out_device_index = i;
				found = true;
				soundio_device_unref(device);
				break;
		...
		if (!found)
			panic("invalid output device id: %s", out_device_id.c_str());
	}
